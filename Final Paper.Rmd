...
We conducted a logistic regression, the beta weights and Wald-z values are reported blow. In the full model it appears that there there is a trend for a negative effect of COMT on likelihood of using a substance,and a positive interaction with income. The $\chi^2$ likeihood ratio test reveals that there is no main effect of COMT alone, but the interaction is trending towards significance. 
```{r,results='asis',echo=FALSE}
...
We conducted a similar regression with DRD2, and found some suggestive evidence that there was a similar interaction. 
