...
However, the risky and behavior and possible poor judgement of those who try dangerous substances clearly does not exist in a vacuum. Our social and economic environment will influence both the availability of psychoactive substances, the messages we learn about the substances, and the magnitude of the consequences for using those substances. There is signifcant debate about whether this is true for adolescents [@hanson2007socioeconomic, @goodman2002socioeconomic]. This suggests that SES might not play a major role in likelihood of using a substance. Another possibility, however, is that SES might only have a protective influence for those who are particularly vulnerable. 
...
.  Their relative proportions are 14:81:56 respectively. For rs12364283, AA= 1, GA = 0 with a relative proportions of 141:10 respectively. Other studies have found a population freqency of roughly 15% for the GA allelic variety [@frank2009genetic], so the minority varient is significantly underrepresented in our population (Binomial test(exact) p = .002). The distribution of the rs12364283 and rs4680 allels appear to be independent $\chi^2$=0.232, p=.896.
...
#Discussion
It appears there is no strong evidence from this data that either SNP had a large influence on likelihood of adolecents and young adults using psycho-active substances. There sems to be some evidence that those with the GA variant of rs12364283 SNP, who are expected to have a  lower denisty of D2 striatal receptors, are more likely to use substances at a higher incomes. It is not clear why this is. Because of the low n, we cannot make strong claims about whether those at lower incomes use less substance than those with the AA variant, or whether they are more likely to use the substance at high incomes. If it is the former, it would suggest that for those in families with lower incomes, the ability to learn to avoid negative sitautions is critical in being able to avoid substance use.

However, the most likely claim is that no effect is significant. It is hard to draw strong conclcusions, because of the limited data ultimately available in this dataset. Together this might provide small evidence that differences in Prefrontal dopamine and striatal D2 receptor density do not explain individual differences in adolescents likelihood of trying an illegal substances, but larger datasets would be needed to truely rule out this effect. 
